	...
	for(i = 0; i< n_S-1; i++){
		...
		switch(feature_case) {
			...
			case WALK:
				...
				rms(data, pos0, pos1, &features[i].seg0.rms);
				rms(data, pos0, pos1, &features[i].seg1.rms);
				rms(data, pos0, pos1, &features[i].seg2.rms);
				rms(data, pos0, pos1, &features[i].seg3.rms);
				break;
		}
	...
	double* time, int* S_i, int n_S, char* ofile_walk_feature_name)
{
	...
	//features1 = extract_feature(accel_y, time, S_i, n_S, WALK);
	...
	for(i = 0; i < n_S-1; i++){
		fprintf(fp, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%20.10lf\n",
		 	features0[i].seg0.max, features0[i].seg0.min, features0[i].seg0.rms, 
			features0[i].seg1.max, features0[i].seg1.min, features0[i].seg1.rms,
			features0[i].seg2.max, features0[i].seg2.min, features0[i].seg2.rms,
			features0[i].seg3.max, features0[i].seg3.min, features0[i].seg3.rms,
			period[i]);
	}
	fclose(fp);
}
